...
class KNNClassifier:
    ...
    def fit(self, X_train, y_train):
        ...
        assert self.k <= X_train.shape[0], "the size of X_train is not at least k."
    ...
    def predict(self, X_predict):
        assert (
            self.X_train_ is not None and self.y_train_ is not None
        ), "must fit before predict"
        assert (
            X_predict.shape[1] == self.X_train_.shape[1]
    ...
    def _predict(self, x):
        assert (
            x.shape[0] == self.X_train_.shape[1]
        ), "the feature number of x must be equal X_train"
