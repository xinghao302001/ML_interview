...
class KMeans:
    ...
    def Nomalize(self, dataArray: np.ndarray):
        """
        ...
        for feature in range(dataArray.shape[1]):
            ...
            for i in range(dataArray.shape[0]):
                dataArray[i][feature] = (dataArray[i][feature] - minfeature) / (
                    maxfeature - minfeature
                )
        return dataArray
    ...
    def _cacul_clusterCenter(self, cluster: List, dataArr: np.ndarray) -> np.ndarray:
        ...
        for i in range(dataArr.shape[1]):
            for n in cluster:
                center[i] += dataArr[n][
                    i
                ]  # caculate the sum of the i-th features in current cluster
        center = center / dataArr.shape[0]  # # caculate the mean for each feature

        return center

    def _Adjusted_Rand_Index(
        self, cluster_dict: Dict, labels: List[int], k: int
    ...
    def kmeans(
        self, dataArr: np.ndarray, k: int, iter_steps: int, labels: List[int]
    ) -> Union[Dict, List]:
