...
def loadDataForMniST(dir_path: str = None):
    """
...
def loadDataForText(dir_path: str) -> List[str]:
    artical = []
...
def loadDataForIris(dir_path: str) -> Union[List, np.ndarray]:
    feature_list = []
    label_list = []
    fr = open(dir_path)
...
def loadDataForCars(dir_path: str) -> np.ndarray:
...
def loadDataForBBC(dir_path: str) -> List:
    df = pd.read_csv(dir_path)
